...
//                    BTreePage temp = (BTreePage) this.getPage(tid, dirtypages, next.getLeftChild(), Permissions.READ_ONLY);
//                    if (temp.getId().pgcateg() == BTreePageId.LEAF) {
//                        return (BTreeLeafPage) getPage(tid,dirtypages,temp.getId(),perm);
//                    } else {
//                        page = (BTreeInternalPage) temp;
//                        it = page.iterator();
//                    }
//                }
//            } else {
...
//                    BTreeEntry next = it.next();
//                    BTreePage temp=null;
//                    while (it.hasNext()&&f.compare(Op.GREATER_THAN, next.getKey())) {
//                    }
//                    if(it.hasNext()) temp = (BTreePage) this.getPage(tid, dirtypages, next.getLeftChild(), Permissions.READ_ONLY);
//                    else temp = (BTreePage) this.getPage(tid, dirtypages, next.getRightChild(), Permissions.READ_ONLY);
//                    if (temp.getId().pgcateg() == BTreePageId.LEAF) {
//                        return (BTreeLeafPage) getPage(tid,dirtypages,temp.getId(),perm);
//                    }else{
//                        page= (BTreeInternalPage) temp;
...
//        return null;
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        Field middleKey = it.next().getField(page.keyField);

        BTreePageId temp = page.getRightSiblingId();
        if (temp != null) {
            BTreeLeafPage tempPage = (BTreeLeafPage) getPage(tid, dirtypages, temp, Permissions.READ_WRITE);
            newPage.setRightSiblingId(temp);
            tempPage.setLeftSiblingId(newPage.getId());
        }
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        int targetEntries = (curEntries + leftSiblingNumEntries) / 2;
        System.out.println("steal from left Internal Page "+" curEntries: "+curEntries+" targetEntries: "+targetEntries);
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        int targetEntries = (curEntries + rightSiblingNumEntries) / 2;
//        System.out.println("steal from right Internal Page "+" curEntries: "+curEntries+"rightEntries: "+rightSiblingNumEntries+" targetEntries: "+targetEntries);
