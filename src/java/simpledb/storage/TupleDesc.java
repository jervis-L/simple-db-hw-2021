...
public class TupleDesc implements Serializable {
    ...
    public int fieldNameToIndex(String name) throws NoSuchElementException {
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).fieldName!=null&&items.get(i).fieldName.equals(name)) return i;
        }
        throw new NoSuchElementException();
    }
    ...
    public static TupleDesc merge(TupleDesc td1, TupleDesc td2) {
        ...
        td.items.addAll(td1.items);
        td.items.addAll(td2.items);
        return td;
    }
    ...
    public boolean equals(Object o) {
        ...
        if (td.numFields() != numFields()) return false;
        for (int i = 0; i < numFields(); i++)
            if (items.get(i).fieldType != td.items.get(i).fieldType)
                return false;
        return true;
    }
